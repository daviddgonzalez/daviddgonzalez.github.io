  ...
  return (
    <article className="lego-card brick-card rounded-[var(--radius-brand)] bg-surface p-6 shadow-sm">
      ...
      <div className="mt-4 flex gap-4 text-sm">
        {project.links.demo && <a className="text-accent underline-offset-2 transition hover:underline hover:opacity-80" href={project.links.demo}>Demo</a>}
        {project.links.repo && <a className="text-accent underline-offset-2 transition hover:underline hover:opacity-80" href={project.links.repo}>Repo</a>}
      </div>
    </article>
  );
}
